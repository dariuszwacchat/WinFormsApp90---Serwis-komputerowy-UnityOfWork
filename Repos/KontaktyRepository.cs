...
{
    public class KontaktyRepository : IDataRepository<Kontakt>
    {
        private ApplicationDbContext _context;

        public KontaktyRepository (ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        {
            return _context.Kontakty
                .Include (i=> i.Firma)
                .ToList();
        }

        public Kontakt Get (string id)
        {
            var model = _context.Kontakty
                .Include (i=> i.Firma)
                .FirstOrDefault (f=> f.KontaktId == id);
            return model;
        }

        public void Create (Kontakt model)
        {
            if (model != null)
            {
                try
                {
                    _context.Kontakty.Add(model);
        ...
        public void Delete (string id)
        {
            try
            {
                var model = _context.Kontakty.FirstOrDefault (f=>f.KontaktId == id);
                if (model != null)
                {
                    _context.Kontakty.Remove(model);
                    _context.SaveChanges();
                }
                else
                {
        ...
        public void Update (Kontakt model)
        {
            if (model != null)
            {
                try
